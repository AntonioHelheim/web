...
$sql = "SELECT 
    ...
    DATE_FORMAT(FROM_UNIXTIME(A.date_added), '%d/%m/%Y') as FecCre, 
    ...
    <table id="tablaActividades" class="table table-dark table-bordered table-hover">
      ...
      <tbody>
        ...
        <tr data-bs-toggle="collapse" data-bs-target="#detalle<?= $i ?>" class="accordion-toggle">
          ...
          <td><?= $row['EntornoReal'] ?></td>
        </tr>
        <tr class="collapse collapse-row" id="detalle<?= $i ?>">
          <td colspan="4">
            <strong>Objetivo:</strong> <?= htmlspecialchars($row['Objetivo']) ?><br>
            <strong>Fecha Ejecución:</strong> <?= $row['FecEjec'] ?><br>
            <strong>Entorno Esperado:</strong> <?= $row['EntornoEsperado'] ?><br>
            <strong>Monto:</strong> <?= $row['Monto'] ?><br>
            <strong>Fuente Financiamiento:</strong> <?= $row['FuenteFinanciamiento'] ?><br>
            ...
            <strong>Mecanismo:</strong> <?= $row['Mecanismo'] ?>
          </td>
        </tr>
        <?php endwhile; ?>
        <?php else: ?>
