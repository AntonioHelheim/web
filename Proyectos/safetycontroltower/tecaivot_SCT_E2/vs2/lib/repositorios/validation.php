...
/**
 * Normaliza un RUT chileno válido a formato 12345678-5 / 12345678-K.
 * Devuelve null si el dígito verificador no es correcto.
 */
function normalizarRutChileno(string $rut): ?string
{
    $limpio = strtoupper(str_replace(['.', '-', ' '], '', trim($rut)));
    if (!validarRutChileno($limpio)) {
        return null;
    }
