-- =====================================================================
-- MIGRACION safetyco_SCT — Safety Control Tower (Helheim / Tecaivot)
-- =====================================================================
--
-- Origen : base "tecaivot_SCT", servidor 201.148.105.87
-- Destino: base "safetyco_SCT", servidor 201.148.104.98
-- Generado a partir del export real del 24-08-2026 (mismo dump que
-- documenta el Anexo "Diccionario de base de datos" del plan de trabajo),
-- con los siguientes ajustes respecto del original:
--
--   1) Se omiten a propósito los datos de login_attempts y login_codes:
--      son datos transitorios/de seguridad de la sesión anterior
--      (intentos de login, códigos OTP ya usados/expirados) — no hace
--      sentido migrarlos, el servidor nuevo arranca esas dos tablas
--      vacías (la estructura sí se crea, para que el login siga
--      funcionando igual desde el primer momento).
--
--   2) Se agrega al final un bloque "BACKFILL DE ROLES" que siembra los
--      roles administrador_completo y jefatura para toda empresa que ya
--      exista y todavía no los tenga. Esto es una migración de datos,
--      no de estructura: el dump original (24-ago) solo contemplaba 3
--      roles por empresa (administrador, cliente, trabajador); el
--      código ya pasó a un esquema de 5 roles pero nunca se sembraron
--      los 2 nuevos para las empresas creadas antes de ese cambio.
--      El bloque es idempotente (usa WHERE NOT EXISTS), se puede
--      correr más de una vez sin duplicar filas.
--
-- PASOS PARA APLICAR ESTA MIGRACIÓN EN EL SERVIDOR NUEVO:
--
--   1. Crear la base de datos `safetyco_SCT` en 201.148.104.98 (cPanel
--      > MySQL Databases, o `CREATE DATABASE safetyco_SCT CHARACTER SET
--      utf8mb4 COLLATE utf8mb4_unicode_ci;` si se tiene acceso directo).
--   2. Crear el usuario de MySQL para esa base y asignarle todos los
--      privilegios sobre ella (este script no crea usuarios).
--   3. Configurar en el hosting nuevo las variables de entorno
--      DB_HOST=201.148.104.98, DB_NAME=safetyco_SCT, DB_USER y DB_PASS
--      con las credenciales del paso 2 (config.php ya está actualizado
--      para usar estos valores por defecto en host/nombre de base, pero
--      exige usuario/password por variable de entorno — no trae
--      credenciales de ningún servidor hardcodeadas).
--   4. Importar este archivo completo contra `safetyco_SCT` (phpMyAdmin
--      > Importar, o `mysql -h 201.148.104.98 -u <usuario> -p
--      safetyco_SCT < safetyco_SCT(vs1).sql`).
--   5. Verificar login con una de las cuentas ya existentes (ej. Juan
--      Antonio) antes de dar la migración por cerrada.
--
-- =====================================================================
...
--
-- Volcado de datos para la tabla `login_attempts`: se omite a propósito en
-- esta migración (datos transitorios/de seguridad de la sesión anterior,
-- arranca vacía en el servidor nuevo).
--
...
--
-- Volcado de datos para la tabla `login_codes`: se omite a propósito en
-- esta migración (datos transitorios/de seguridad de la sesión anterior,
-- arranca vacía en el servidor nuevo).
--
...
--
ALTER TABLE `login_codes`
  MODIFY `id_login_code` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=1;
...
--
ALTER TABLE `users_role_group`
  MODIFY `id_role_group` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=8;
...
--
-- =====================================================================
-- BACKFILL DE ROLES — administrador_completo y jefatura
-- =====================================================================
-- Siembra estos 2 roles para toda empresa que ya exista en `company` y
-- todavía no los tenga en `users_role_group` (ver punto 2 del comentario
-- inicial de este archivo). Idempotente: se puede correr más de una vez.
--

INSERT INTO `users_role_group` (`id_company`, `name`, `description`, `state`, `create_by`, `date_create`, `last_update`)
SELECT c.id_company, 'administrador_completo', 'Administrador global', 1, 'migracion_safetyco', NOW(), NOW()
FROM `company` c
WHERE NOT EXISTS (
    SELECT 1 FROM `users_role_group` urg
    WHERE urg.id_company = c.id_company AND urg.name = 'administrador_completo'
);

INSERT INTO `users_role_group` (`id_company`, `name`, `description`, `state`, `create_by`, `date_create`, `last_update`)
SELECT c.id_company, 'jefatura', 'Jefatura de empresa', 1, 'migracion_safetyco', NOW(), NOW()
FROM `company` c
WHERE NOT EXISTS (
    SELECT 1 FROM `users_role_group` urg
    WHERE urg.id_company = c.id_company AND urg.name = 'jefatura'
);
