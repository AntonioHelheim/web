...
requireCapability($pdo, 'events.create');
...
try {
    $auditData = [
        'id_company'=>$idCompany,'id_company_center'=>$idCenter,'id_project'=>$idProject,'module'=>'seguridad',
        'id_worker'=>$idWorker,'id_worker_name'=>$idWorkerName,'id_event'=>$idEvent,
        'event_date'=>$fechaEvento->format('Y-m-d H:i:s'),'description'=>$description,'criticality'=>$criticality,'state'=>1
    ];
    $idNuevo = eventoCrear($pdo, $auditData, currentUserId());
    auditTrailLogChanges($pdo, $idCompany, 'eventos', 'security_events', $idNuevo, [], $auditData,
        'create', 'Evento #' . $idNuevo);
