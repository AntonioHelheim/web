...
requireCapability($pdo, 'events.edit');
...
try {
    ...
    $nuevosDatos = [
        'id_company_center'=>$idCenter,'id_project'=>$idProject,'id_worker'=>$idWorker,'id_worker_name'=>$idWorkerName,
        'id_event'=>$idEvent,'event_date'=>$fechaEvento->format('Y-m-d H:i:s'),'description'=>$description,'criticality'=>$criticality
    ];
    eventoActualizar($pdo, $idEvento, $nuevosDatos);
    auditTrailLogChanges($pdo, $idCompany, 'eventos', 'security_events', $idEvento, $evento,
        array_merge($evento,$nuevosDatos), 'update', 'Evento #' . $idEvento,
        ['id_company_center','id_project','id_worker','id_worker_name','id_event','event_date','description','criticality']);
