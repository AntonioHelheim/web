...
requireCapability($pdo, 'projects.edit');
...
try {
    ...
    $nuevosDatos = ['name' => $name, 'description' => $description];
    proyectoActualizar($pdo, $idProject, $nuevosDatos);
    auditTrailLogChanges($pdo, (int) $proyecto['id_company'], 'proyectos', 'projects', $idProject,
        $proyecto, array_merge($proyecto, $nuevosDatos), 'update', $name, ['name','description']);
