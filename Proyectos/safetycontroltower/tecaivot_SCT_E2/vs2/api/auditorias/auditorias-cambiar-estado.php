...
requireCapability($pdo, 'audits.state');
...
try {
    ...
    auditTrailLogAction($pdo, (int)$test['id_company'], 'auditorias', 'company_test', $idTest, 'state',
        $test['state'], $state, 'state_change', (string)$test['name']);
    responderJSON(true, ['state' => $state], $state === 1 ? 'Auditoría reactivada.' : 'Auditoría desactivada.');
} catch (PDOException $e) {
    error_log('api/auditorias/auditorias-cambiar-estado.php: ' . $e->getMessage());
    responderJSON(false, null, 'No se pudo cambiar el estado.', 500);
}
