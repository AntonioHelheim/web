...
if (!$idRel) responderJSON(false, null, 'Relación inválida.', 400);

try {
    $rel = cursoObtenerRelPorId($pdo, $idRel);
    if (!$rel) responderJSON(false, null, 'Pregunta no encontrada en la auditoría.', 404);
    $test = cursoObtenerPorId($pdo, (int) $rel['id_test']);
    ...
    if (auditoriaTestTieneAsignaciones($pdo, (int) $test['id_test'])) {
    ...
    if (cursoPreguntaTieneRespuestas($pdo, $idRel)) {
        responderJSON(false, null, 'No se puede quitar una pregunta que ya tiene respuestas registradas.', 409);
    }
    cursoQuitarPregunta($pdo, $idRel);
    auditTrailLogAction($pdo, (int)$test['id_company'], 'auditorias', 'company_test_rel_questions', $idRel,
        'id_question', $rel['id_question'] ?? null, null, 'unassign', (string)$test['name']);
    responderJSON(true, null, 'Pregunta quitada.');
} catch (PDOException $e) {
    error_log('api/auditorias/auditoria-preguntas-quitar.php: ' . $e->getMessage());
    responderJSON(false, null, 'No se pudo quitar la pregunta.', 500);
}
