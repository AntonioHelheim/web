...
requireCapability($pdo, 'centers.create');
...
try {
    ...
    $nuevosDatos = ['id_company'=>$idCompany,'name'=>$name,'description'=>$description,'state'=>1];
    $idNuevo = centroCrear($pdo, $nuevosDatos, currentUserId());
    auditTrailLogChanges($pdo, $idCompany, 'centros', 'company_center', $idNuevo, [], $nuevosDatos, 'create', $name);
