...
if (!empty($perfil['name'])) {
    $iniciales .= sctTextSubstr((string) $perfil['name'], 0, 1);
}
if (!empty($perfil['lastname'])) {
    $iniciales .= sctTextSubstr((string) $perfil['lastname'], 0, 1);
}
if ($iniciales === '') {
    $iniciales = sctTextSubstr((string) ($_SESSION['user_email'] ?? 'US'), 0, 2);
}
