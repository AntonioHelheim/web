<?php
/**
 * POST /api/induccion/preguntas-editar.php
 * Mismo body que crear.php + id_questions. Reemplaza todas las
 * alternativas (ver comentario en InduccionRepository::preguntaActualizar).
 */
...
try {
    ...
    if (!$pregunta) {
        responderJSON(false, null, 'Pregunta no encontrada.', 404);
    }

    preguntaActualizar($pdo, $idPregunta, [
        'question'          => $question,
    ...
    responderJSON(true, null, 'Pregunta actualizada correctamente.');
} catch (PDOException $e) {
    error_log('api/induccion/preguntas-editar.php: ' . $e->getMessage());
    responderJSON(false, null, 'No se pudo actualizar la pregunta.', 500);
}
