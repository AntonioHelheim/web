...
requireCapability($pdo, 'self_assessments.create');
if ($_SERVER['REQUEST_METHOD'] !== 'POST') responderJSON(false, null, 'Método no permitido.', 405);
$input = json_decode(file_get_contents('php://input'), true);
if (!is_array($input)) $input = $_POST;
requireCsrfToken($input);

$idCompany = autoevaluacionResolveCompanyId($pdo, isset($input['id_company']) ? (int) $input['id_company'] : null);
...
try {
    $stmt = $pdo->prepare('SELECT 1 FROM company_test WHERE id_company=:id_company AND type=:type AND name=:name AND state=1 LIMIT 1');
    $stmt->execute(['id_company'=>$idCompany,'type'=>AUTOEVALUACION_TIPO,'name'=>$name]);
    if ($stmt->fetchColumn()) responderJSON(false, null, 'Ya existe una autoevaluación activa con ese nombre en la empresa.', 409);

    $id = cursoCrear($pdo, [
        'id_company'=>$idCompany, 'name'=>$name, 'type'=>AUTOEVALUACION_TIPO,
        'description'=>$description, 'attempts_allowed'=>$attemptsAllowed,
    ...
    auditTrailLogChanges($pdo, $idCompany, 'autoevaluaciones', 'company_test', $id, [], [
        'name'=>$name,'type'=>AUTOEVALUACION_TIPO,'description'=>$description,'attempts_allowed'=>$attemptsAllowed,
    ...
    responderJSON(true, ['id_test'=>$id], 'Autoevaluación creada correctamente.', 201);
} catch (PDOException $e) {
    error_log('api/autoevaluaciones/autoevaluaciones-crear.php: '.$e->getMessage());
    responderJSON(false, null, 'No se pudo crear la autoevaluación.', 500);
}
