...
requireCapability($pdo, 'events.tracking');
...
try {
    ...
    if ((int) $evento['state'] === EVENTO_ABIERTO) {
        ...
        auditTrailLogAction($pdo, (int)$evento['id_company'], 'eventos', 'security_events', $idEvento,
            'state', $evento['state'], EVENTO_EN_PROCESO, 'state_change', 'Evento #' . $idEvento);
    }
    auditTrailLogAction($pdo, (int)$evento['id_company'], 'eventos', 'security_event_tracking', $idNuevo,
        'tracking_description', null, $descripcion, 'track', 'Evento #' . $idEvento);
