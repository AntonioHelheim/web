...
class OverworldScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.events.on('resume', (sys, data) => {
      ...
      if (data && data.newAppearance) {
        this.appearance = data.newAppearance;
        this.player.setAppearance(data.newAppearance);
      }
    });
    this.events.on('shutdown', () => { if (this.mpTimer) this.mpTimer.remove(); });
  }

  // Phaser NO desactiva automáticamente el teclado de una escena pausada:
  // sin esto, la escena de fondo seguía "escuchando" teclas al mismo tiempo
  // que el menú o la batalla de encima, causando comportamiento errático
  // (por eso el menú no funcionaba bien). Se reactiva en el handler 'resume'.
  pauseForOverlay(sceneKey, data) {
  ...
  openMenu() {
    this.pauseForOverlay('MenuScene', { overworldScene: this });
  }
  ...
  startEncounter() {
    ...
    this.pauseForOverlay('BattleScene', { playerMonster: this.party[0], enemyMonster: enemy, overworldScene: this });
  }
  ...
  enterPvpBattle(battleId) {
    this.pendingChallengeFromMe = null;
    this.pauseForOverlay('PvpBattleScene', { battleId, overworldScene: this });
  }
}
