...
class Player {
  ...
  // Simula el paso al caminar: el cuerpo rebota un poco hacia arriba y las
  // piernas se mueven en tijera (una adelante, otra atrás), alternando de
  // lado en cada paso. Sin esto el personaje solo se deslizaba sin dar la
  // sensación de estar caminando.
  animateWalkStep() {
    ...
    const { legLeft, legRight } = this.visual;
    if (!legLeft || !legRight) return;
    const forwardLeg = this.stepPhase === 'left' ? legLeft : legRight;
    const backLeg = this.stepPhase === 'left' ? legRight : legLeft;
    const swing = ts * 0.05;

    this.scene.tweens.add({
      targets: forwardLeg, y: forwardLeg.y - swing, duration: 75, yoyo: true, ease: 'Sine.easeInOut',
    });
    this.scene.tweens.add({
      targets: backLeg, y: backLeg.y + swing, duration: 75, yoyo: true, ease: 'Sine.easeInOut',
    });
  }
}
