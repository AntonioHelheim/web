...
function buildCharacterVisual(scene, tileSize, appearance) {
  ...
  if (isGirl) {
    // Piernas apenas visibles bajo el vestido.
    parts.push(scene.add.rectangle(-tileSize * 0.07, tileSize * 0.38, tileSize * 0.08, tileSize * 0.1, skin));
    parts.push(scene.add.rectangle(tileSize * 0.07, tileSize * 0.38, tileSize * 0.08, tileSize * 0.1, skin));
    ...
    // Piernas.
    parts.push(scene.add.rectangle(-tileSize * 0.09, tileSize * 0.36, tileSize * 0.13, tileSize * 0.22, outfitShade));
    parts.push(scene.add.rectangle(tileSize * 0.09, tileSize * 0.36, tileSize * 0.13, tileSize * 0.22, outfitShade));
    // Zapatos.
    parts.push(scene.add.ellipse(-tileSize * 0.09, tileSize * 0.46, tileSize * 0.15, tileSize * 0.07, 0x3b2513));
    parts.push(scene.add.ellipse(tileSize * 0.09, tileSize * 0.46, tileSize * 0.15, tileSize * 0.07, 0x3b2513));
    // Torso.
    parts.push(scene.add.ellipse(0, tileSize * 0.16, tileSize * 0.4, tileSize * 0.36, outfit));
  }

  // Brazos: bultos redondeados a los costados del torso, en vez de un bloque rígido.
  parts.push(scene.add.circle(-tileSize * 0.22, tileSize * 0.1, tileSize * 0.09, outfit));
  parts.push(scene.add.circle(tileSize * 0.22, tileSize * 0.1, tileSize * 0.09, outfit));
