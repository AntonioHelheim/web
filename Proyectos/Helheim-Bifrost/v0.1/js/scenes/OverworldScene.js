...
// Colores estables por jugador, derivados de un hash simple del nombre de
// usuario, para que cada quien se vea siempre del mismo color.
const REMOTE_COLORS = [0xd94f2b, 0x3a6ea5, 0xf2c14e, 0x9b5de5, 0x2bb673, 0xe0459f];
...
class OverworldScene extends Phaser.Scene {
  ...
  init(data) {
    ...
    this.startTile = { x: data.x, y: data.y };
  ...
  create() {
    ...
    this.player = new Player(this, this.startTile.x, this.startTile.y, TILE_SIZE);
    const myLabel = this.add.text(0, -TILE_SIZE * 0.65, window.POKEWEB_USER.username, {
    ...
    this.hint = this.add.text(6, 6, 'Flechas: moverse | S: guardar | R: retar a jugador cercano', {
    ...
    this.events.on('resume', () => this.statusText.setText(''));
  ...
  drawMap() {
    ...
    for (let y = 0; y < layout.length; y++) {
      for (let x = 0; x < layout[y].length; x++) {
        ...
        this.add.rectangle(worldX, worldY, TILE_SIZE, TILE_SIZE, this.colorForTile(layout[y][x])).setDepth(0);
      }
    }
  }
  ...
  colorForTile(tile) {
    switch (tile) {
      case 1: return GB_PALETTE.dark;
      case 2: return GB_PALETTE.darkest;
      case 3: return GB_PALETTE.water;
      case 4: return GB_PALETTE.lightest;
      default: return GB_PALETTE.light;
    }
  }
  ...
  changeMap(mapKey, spawn) {
    this.scene.restart({ mapKey, x: spawn.x, y: spawn.y, party: this.party, inventory: this.inventory });
  }
  ...
  startEncounter() {
    if (this.party.length === 0) {
      this.party.push(makeMonsterInstance(randomSpeciesKey()));
    }
    const enemy = makeMonsterInstance(randomSpeciesKey());
    this.scene.pause();
    this.scene.launch('BattleScene', { playerMonster: this.party[0], enemyMonster: enemy, overworldScene: this });
  }
  ...
  updateRemotePlayers(players) {
    ...
    for (const p of players) {
      ...
      if (!remote) {
        remote = this.createRemotePlayer(p.username);
        remote.container.setPosition(worldX, worldY);
        this.remotePlayers[p.username] = remote;
      } else if (remote.tileX !== p.x || remote.tileY !== p.y) {
  ...
  createRemotePlayer(username) {
    ...
    const body = this.add.rectangle(0, 0, TILE_SIZE * 0.7, TILE_SIZE * 0.7, color);
    body.setStrokeStyle(2, GB_PALETTE.darkest);
    const label = this.add.text(0, -TILE_SIZE * 0.65, username, {
      fontFamily: 'monospace', fontSize: '9px', color: '#0f380f',
      backgroundColor: '#9bbc0f', padding: { x: 2, y: 1 },
    }).setOrigin(0.5, 1);
    const container = this.add.container(0, 0, [body, label]);
    container.setDepth(9);
    return { container, tileX: 0, tileY: 0 };
  }
  ...
  enterPvpBattle(battleId) {
    this.pendingChallengeFromMe = null;
    this.scene.pause();
    this.scene.launch('PvpBattleScene', { battleId, overworldScene: this });
  }
}
