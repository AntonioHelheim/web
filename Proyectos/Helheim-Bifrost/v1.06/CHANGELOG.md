...
### 🧹 Pulido: pruebas ad-hoc consolidadas en npm test (05-09-2026)
Durante el desarrollo de terreno/edificios/personajes/niveles se
escribieron muchas pruebas puntuales con mocks para verificar cada
cosa antes de integrarla — pero vivían como scripts sueltos en `/tmp`,
fuera del proyecto, así que ninguna corría automáticamente ni protegía
contra una regresión futura. Se consolidaron en 3 scripts permanentes:
`scripts/test-tile-rendering.js` (41 verificaciones: terreno, agua,
árbol, puertas, roca, pared, piso de cueva, decoraciones — cada una con
y sin su textura real), `scripts/test-character-visuals.js` (27
verificaciones: detección de spritesheet mal dimensionado, las 8
combinaciones de personaje, escala visual por género), y
`scripts/test-preload-regression.js` (regresión específica del bug real
de characterCreated perdido). `npm test` ahora corre 8 suites en total
con un solo comando.


### ✨ Nuevo: niveles interiores del Gran Árbol y la Cueva de Don Emilio
9 mapas nuevos — Gran Árbol (`tree_hub` + 3 niveles arriba + 3 abajo) y
Cueva de Don Emilio (2 niveles abajo) — con 10 criaturas de combate
nuevas (8 Battlers, un dragón guardián de la cima, un demonio en el
fondo). Encuentros restringidos por mapa (`data/map-encounters.json`),
sprites de batalla reales cuando existen. Ver la sección dedicada en
`PLAN-GRAPHICS-AUDIO.md` para el detalle completo — incluye 2 problemas
reales encontrados y corregidos en el camino (acceso a la cueva
bloqueado por roca, y un problema de seguridad propio donde el mapKey
del cliente no debía usarse directamente para elegir al enemigo).


### 🐛 Corregido: la conexión a la base de datos se rompía en cada entrega
El usuario reportó problemas de conexión tanto en localhost como en su
hosting real después de recibir una versión nueva. Causa encontrada:
`api/config.php` trae credenciales **genéricas** a propósito
(`TU_USUARIO_bifrost`, `TU_CONTRASEÑA_AQUI` para producción) — el
usuario había editado ese mismo archivo a mano con sus credenciales
reales de cPanel, y cada entrega nueva del proyecto sobreescribía ese
archivo completo, perdiendo la edición.

**Solución:** `api/config.php` ahora intenta cargar
`api/config.local.php` (opcional) antes de definir cualquier valor por
defecto — ese archivo nunca se entrega ni se vuelve a generar (está en
`.gitignore`), así que las credenciales reales sobreviven a cualquier
actualización futura. Se agregó `api/config.local.example.php` como
plantilla (sin credenciales reales) y una sección nueva en
`debug-entorno.php` que confirma si el archivo existe y qué credenciales
está usando en ese momento — para que este problema sea inmediatamente
visible la próxima vez, en vez de manifestarse como un misterioso error
de conexión.

Se encontró y corrigió además un detalle en el mecanismo: la primera
versión verificaba las 4 constantes (`DB_HOST`/`DB_NAME`/`DB_USER`/
`DB_PASS`) como un bloque conjunto (`if (!defined('DB_HOST')) { ...
las 4 ... }`), lo que rompía la configuración si `config.local.php`
definía solo algunas — corregido para verificar cada una por separado.

**⚠️ Nota de seguridad:** en el mensaje donde se reportó este problema,
se compartió sin querer una contraseña real de base de datos en el
chat — se le recomendó al usuario rotarla en cPanel de inmediato.
