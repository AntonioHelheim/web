...
class OverworldScene extends Phaser.Scene {
  ...
  init(data) {
    ...
    this.inventory = data.inventory || { runa_captura: 5 };
    this.appearance = data.appearance || { gender: 'boy', preset: 1, skinColor: '#f1c27d', hairColor: '#2c1b18', eyeColor: '#3b2415' };
  }
  ...
  // NPCs ambientales por mapa — posiciones y sprite definidos en
  // data/npc-spawns.json (ítem 4 del roadmap: data-driven, no
  // hardcodeado acá), leído por PreloadScene.js hacia el global
  // NPC_SPAWNS. De momento sin diálogo ni interacción (ítem 8, todavía
  // bloqueado por contenido narrativo) — solo dan vida ambiental.
  spawnAmbientNPCs() {
    const spawns = NPC_SPAWNS[this.mapKey] || [];
    this.ambientNPCs = spawns.map(
      (s) => new NPC(this, this.mapKey, s.homeX, s.homeY, s.spriteKey, s.wanderRadius)
    );
  }
  ...
  // Mobiliario/decoración de interiores (escritorio, cama, símbolos) —
  // dibujo a mano (drawDecoration en TileVisuals.js), puramente visual,
  // no bloquea el paso. Mismo patrón de datos que landmarks.
  drawDecorations() {
    const decorations = MAPS[this.mapKey].decorations || [];
    decorations.forEach(({ x, y, type }) => {
      const worldX = x * TILE_SIZE + TILE_SIZE / 2;
      const worldY = y * TILE_SIZE + TILE_SIZE / 2;
      drawDecoration(this, worldX, worldY, TILE_SIZE, type);
    });
  }
  ...
  changeMap(mapKey, spawn) {
    // Guardado silencioso (sin mensajes en pantalla, a diferencia de
    // saveGame() con la tecla S) — necesario para que
    // wild_battle_start.php sepa en qué nivel está el jugador REALMENTE
    // parado al calcular qué criatura salvaje aparece (lee saves.map_key
    // de la base de datos, no lo que mande el cliente en el momento de
    // la pelea). Sin esto, cruzar a un nivel nuevo del Gran Árbol/cueva
    // sin haber guardado con "S" antes dejaría el pool de encuentros
    // desactualizado. Falla en silencio si el backend no responde —
    // no debe bloquear ni interrumpir la transición de mapa.
    fetch('api/save_game.php', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
      body: JSON.stringify({
        mapKey, x: spawn.x, y: spawn.y,
        party: this.party, inventory: this.inventory,
  ...
  async startEncounter() {
    ...
    // avisa que hay algo en la hierba, sin iniciar batalla. (El servidor
    // también valida esto — este chequeo es solo para responder al
    // instante sin esperar una ida y vuelta al servidor.)
    if (this.party.length === 0) {
      this.statusText.setText('Algo se movió en la hierba... pero no tienes compañeros todavía.');
      return;
    }

    // El enemigo y el estado inicial los decide el servidor (ítem 3 de
    // ROADMAP-ARQUITECTURA.md) — ya no se genera ni se calcula nada de la
    // batalla acá, solo se pide y se transiciona con lo que responda.
    try {
      const res = await fetch('api/wild_battle_start.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
      ...
      if (!res.ok || !data.ok) {
        this.statusText.setText(data.error || 'No se pudo iniciar la batalla.');
        return;
      }
      this.handoffTo('BattleScene', { battleId: data.battleId, you: data.you, enemy: data.enemy, message: data.message });
    } catch (err) {
      this.statusText.setText('No se pudo conectar con el servidor.');
    }
  }
    ...
    for (const p of players) {
      ...
      if (!remote) {
        ...
        if (remote.facing !== p.facing) {
          if (remote.usingRealSprite) {
            remote.visual.setFrame(idleFrameForDirection(p.facing));
          } else {
  ...
  createRemotePlayer(username, appearance, facing) {
    const resolved = appearance || { gender: 'boy', preset: 1, skinColor: '#f1c27d', hairColor: '#2c1b18', eyeColor: '#3b2415' };
    const initialFacing = facing || 'down';
    const spriteKey = spriteKeyForAppearance(resolved);
    const usingRealSprite = this.textures.exists(spriteKey);
    // Mismo detalle visual que el jugador local (ver Player.buildVisual):
    // personajes con apariencia masculina se ven 10% más grandes.
    const escala = scaleForGender(resolved.gender);
    ...
    if (usingRealSprite) {
      ...
      visual = this.add.sprite(0, yOffset, spriteKey, idleFrameForDirection(initialFacing));
      visual.setScale(escala);
    } else {
      visual = buildCharacterVisual(this, TILE_SIZE, resolved);
      applyFacingToVisual(visual, initialFacing);
      visual.setScale(escala);
    }
    ...
    return { container, visual, spriteKey, usingRealSprite, tileX: 0, tileY: 0, facing: initialFacing };
  }
  ...
  async tryChallengeNearby() {
    ...
    try {
      const res = await fetch('api/challenge_send.php', {
        ...
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ toUsername: target, csrf_token: window.BIFROST_CSRF_TOKEN }),
      });
      const data = await res.json();
      if (!res.ok) {
  ...
  async respondToIncomingChallenge(accept) {
    ...
    try {
      const res = await fetch('api/challenge_respond.php', {
        ...
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ challengeId, accept, csrf_token: window.BIFROST_CSRF_TOKEN }),
      });
      const data = await res.json();
