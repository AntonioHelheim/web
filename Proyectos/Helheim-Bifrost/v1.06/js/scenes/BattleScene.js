...
class BattleScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.enemySprite = this.buildCombatantVisual(w * 0.72, h * 0.28, this.enemy, 44);
    this.playerSprite = this.buildCombatantVisual(w * 0.25, h * 0.68, this.you, 40, true);
  ...
  // Muestra el sprite real de batalla de la criatura si existe (ver
  // PLAN-GRAPHICS-AUDIO.md — Battlers/dragón/demonio, 05-09-2026), cayendo
  // al círculo/rectángulo de color de siempre si no hay uno cargado. Solo
  // el enemigo trae `speciesKey` por ahora (siempre viene de
  // random_monster() en el servidor) — el equipo del jugador todavía no
  // referencia species.json, así que esValor esFallback siempre da
  // rectángulo para "you" hasta que exista ese vínculo.
  buildCombatantVisual(x, y, monster, size, esJugador = false) {
    const textureKey = monster.speciesKey ? `battle_${monster.speciesKey}` : null;
    if (textureKey && this.textures.exists(textureKey)) {
      return this.add.image(x, y, textureKey).setDisplaySize(size * 2, size * 2).setDepth(1);
    }
    return esJugador
      ? this.add.rectangle(x, y, size, size, monster.color).setDepth(1)
      : this.add.circle(x, y, size * 0.55, monster.color).setDepth(1);
  }
