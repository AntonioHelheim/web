...
// Terreno base (camino, pasto alto, arena) — imágenes de tipo "autotile"
// con muchas variantes/bordes (no solo un cuadrado plano), como agua o
// personajes) — en vez de intentar un sistema de blending completo (un
// proyecto en sí mismo), se usa un ÚNICO recorte fijo de 32x32 ya
// verificado visualmente como relleno sólido y parejo en cada hoja, el
// mismo criterio de "textura real simple" que ya usamos con el agua.
// Si la textura no está cargada, cae al dibujo a mano de siempre.
const TILE_TERRAIN_TEXTURES = {
  0: { key: 'tile_terrain_earth', cropX: 32, cropY: 32 }, // camino
  1: { key: 'tile_terrain_grass', cropX: 32, cropY: 32 }, // pasto alto
  7: { key: 'tile_terrain_sand', cropX: 32, cropY: 32 },  // arena
  10: { key: 'tile_terrain_cave_floor', cropX: 32, cropY: 32 }, // piso de interior de cueva
};

function drawRealTerrainTile(scene, wx, wy, ts, config) {
  scene.add.image(wx, wy, config.key)
    .setCrop(config.cropX, config.cropY, 32, 32)
    .setDisplaySize(ts, ts)
    .setDepth(0);
}

function drawPath(scene, wx, wy, ts, seed) {
  if (scene.textures.exists(TILE_TERRAIN_TEXTURES[0].key)) {
    drawRealTerrainTile(scene, wx, wy, ts, TILE_TERRAIN_TEXTURES[0]);
  } else {
...
function drawTallGrass(scene, wx, wy, ts, seed) {
  if (scene.textures.exists(TILE_TERRAIN_TEXTURES[1].key)) {
    drawRealTerrainTile(scene, wx, wy, ts, TILE_TERRAIN_TEXTURES[1]);
  } else {
...
function drawSand(scene, wx, wy, ts, seed) {
  if (scene.textures.exists(TILE_TERRAIN_TEXTURES[7].key)) {
    drawRealTerrainTile(scene, wx, wy, ts, TILE_TERRAIN_TEXTURES[7]);
    return; // los puntitos dibujados a mano de abajo eran para el respaldo; con textura real no hacen falta
  }
...
// Piso de interior de cueva (tile 10 — cave_down_1/2, ver
// PLAN-GRAPHICS-AUDIO.md) — mismo criterio que el resto del terreno:
// recorte fijo ya verificado, respaldo al dibujo a mano (reutiliza el
// look de drawCaveEntrance simplificado) si la textura no carga.
function drawCaveFloor(scene, wx, wy, ts) {
  if (scene.textures.exists('tile_terrain_cave_floor')) {
    drawRealTerrainTile(scene, wx, wy, ts, TILE_TERRAIN_TEXTURES[10]);
  } else {
    scene.add.rectangle(wx, wy, ts, ts, 0x2b1810).setDepth(0);
  }
}

function drawBuildingWall(scene, wx, wy, ts, seed) {
  if (scene.textures.exists('tile_wall_panel')) {
    scene.add.image(wx, wy, 'tile_wall_panel').setDisplaySize(ts, ts).setDepth(0);
  } else if (scene.textures.exists('tile_black')) {
...
// Decoración de interiores (escritorio, cama: imágenes reales; cruz/estrella:
// dibujo a mano, sin archivo específico para esos símbolos todavía — ver
...
function drawDecoration(scene, wx, wy, ts, type) {
  switch (type) {
    case 'dragon': // pieza central del Nido del Dragón — más grande que un tile normal, a propósito imponente
      if (scene.textures.exists('battle_dragon_ancestral')) {
        scene.add.image(wx, wy, 'battle_dragon_ancestral').setDisplaySize(ts * 2.3, ts * 2.3).setDepth(2);
      } else {
        scene.add.circle(wx, wy, ts * 0.9, 0x6a9450).setStrokeStyle(2, 0x3b2513).setDepth(2);
      }
      break;
    case 'demon': // pieza central de la Guarida del Demonio
      if (scene.textures.exists('battle_demonio_menor')) {
        scene.add.image(wx, wy, 'battle_demonio_menor').setDisplaySize(ts * 1.7, ts * 2.5).setDepth(2);
      } else {
        scene.add.triangle(wx, wy, 0, -ts, -ts * 0.8, ts * 0.8, ts * 0.8, ts * 0.8, 0xa02020).setDepth(2);
      }
      break;
    case 'desk': // escritorio de recepción
      if (scene.textures.exists('furniture_desk')) {
        scene.add.image(wx, wy, 'furniture_desk').setDisplaySize(ts * 0.85, ts * 0.85).setDepth(2);
      } else {
      ...
      break;
    case 'bed': // cama — la imagen real es 16x32 (el doble de alta que ancha), se mantiene esa proporción
      if (scene.textures.exists('furniture_bed')) {
        scene.add.image(wx, wy, 'furniture_bed').setDisplaySize(ts * 0.6, ts * 1.2).setDepth(2);
      } else {
...
function drawTile(scene, x, y, ts, tileType) {
  ...
  switch (tileType) {
    ...
    case 10: drawCaveFloor(scene, wx, wy, ts); break;
