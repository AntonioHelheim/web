...
function checkWarps(mapKey, map) {
  ...
  for (const [posicion, warp] of entradas) {
    ...
    // Tile 4 (puerta genérica) y tile 8 (entrada de cueva, con su propio
    // dibujo distintivo de arco — ver drawCaveEntrance en TileVisuals.js)
    // son ambos válidos para un warp; cualquier otro tipo es sospechoso.
    if (map.layout[y][x] !== 4 && map.layout[y][x] !== 8) {
      advertencia(`warp en "${posicion}" no está sobre un tile tipo 4 u 8 (warp) — tiene tipo ${map.layout[y][x]}`);
    }
