...
## ✅ Niveles del Gran Árbol y la Cueva de Don Emilio (05-09-2026)

Pedido: una "micro ciudad de niveles" dentro del Gran Árbol (3 arriba,
3 abajo) y lo mismo simplificado para la cueva (2 abajo), poblados con
las criaturas de combate nuevas (Battlers, dragón, demonio).

### Mapas nuevos (9 en total)
- **Gran Árbol**: `tree_hub` (entrada, conecta a arriba y abajo) →
  `tree_up_1/2/3` (sube) y `tree_down_1/2/3` (baja). Cada nivel es una
  habitación de 10×8 con paredes reales (`tile_wall_panel`), piso mixto
  (camino + pasto alto para encuentros), y una puerta de entrada/salida.
- **Cueva de Don Emilio**: `cave_down_1` → `cave_down_2` (solo hacia
  abajo, como corresponde a una cueva).

Para entrar al árbol hubo que **reducirlo en 1 fila** (de 10×10 a 9×10)
y abrir una puerta en la fila liberada — el árbol se dibuja como una
sola forma compuesta según su caja delimitadora
(`findTile5Regions()`/`drawBigTree()` en el código), así que una puerta
dentro del bloque original habría quedado tapada visualmente por el
propio dibujo del árbol.

**Se encontró un problema real de acceso en el camino:** la entrada de
la Cueva de Don Emilio (tile 8, ya existía como paisaje) estaba
completamente rodeada de roca (tile 6) — nunca fue pensada para
caminarse hasta ahí, era puramente decorativa de fondo. Se abrió un
camino de 4 tiles conectándola con el camino transitable más cercano.

**Validado con `check-maps.js` en cada paso** (mismo cuidado de siempre
tras los 2 errores de sesiones anteriores): conectividad 100% en los 9
mapas nuevos, más 2 problemas reales que el propio validador detectó y
se corrigieron — un warp con spawn de destino sobre tile bloqueado, y
una advertencia legítima que llevó a ampliar el validador para aceptar
tile 8 (entrada de cueva) como posición de warp válida, además de tile 4.

### Criaturas nuevas (10, en `data/species.json`)
8 Battlers (glenys, hex, ley, linail, oratio, ouzo, prime, wyvera) +
Dragón Ancestral + Pequeño Demonio — nombres/tipos asignados mirando
cada sprite real, no a ciegas. El dragón vive solo en `tree_up_3` (cima)
y el demonio solo en `tree_down_3` (fondo), a modo de jefes de zona; los
8 Battlers se reparten en el resto de los niveles nuevos
(`data/map-encounters.json`).

### Sistema de encuentros por mapa (nuevo, reutilizable)
`random_monster($mapKey)` en `config.php` ahora elige del pool
específico del mapa si existe uno en `data/map-encounters.json`; si no,
sigue usando el catálogo completo (comportamiento de siempre, sin
romper nada en el resto del juego). Probado contra MariaDB real: un
jugador ubicado en `tree_up_3` obtiene el dragón en 20/20 intentos; en
`overworld`, las 34 especies siguen apareciendo con normalidad.

**Corregido un problema de seguridad propio en el camino:** el primer
intento pasaba el `mapKey` directo desde el body de la petición del
cliente a `wild_battle_start.php` — esto habría permitido pedir
"pelear en tree_up_3" sin haber llegado ahí de verdad. Se corrigió para
leer `saves.map_key` desde la base de datos (la fuente de verdad ya
usada en todo el proyecto), no lo que mande el cliente. Como
consecuencia, `OverworldScene.changeMap()` ahora guarda la posición en
silencio al cruzar cualquier warp (antes solo se guardaba con la tecla
S) — si no, cruzar a un nivel nuevo sin haber guardado manualmente
habría dejado desactualizado el mapa real del jugador en la base.

### Sprites de batalla reales
`BattleScene.buildCombatantVisual()` muestra el sprite real (primer
cuadro/pose de reposo, no la animación de ataque completa) si existe
para la `speciesKey` del enemigo — cae al círculo de color de siempre
si no. El equipo del propio jugador sigue sin `speciesKey` (no hay
sistema de captura implementado todavía), así que por ahora conserva su
rectángulo de color en cualquier caso. Probado con mock: sprite real,
respaldo a círculo, y respaldo a rectángulo para el jugador — los 3 pasan.

### Piso de cueva real (05-09-2026)
`cave-tiles-32x32.png` (ya verificado, esperando este momento) ahora
texturiza el piso de `cave_down_1`/`cave_down_2` (tile tipo 10 nuevo),
en vez de reusar la tierra/camino genérica del árbol. Mismo criterio de
recorte fijo verificado que el resto del terreno. **Inconsistencia
menor y a propósito:** las zonas de encuentro siguen usando tile 1
(pasto alto) sin cambios, ya que el mecanismo de encuentro salvaje está
atado a ese tipo de tile específicamente — visualmente se ve "pasto"
dentro de una cueva, lo cual no es perfectamente temático, pero cambiar
esto requeriría modificar el mecanismo de encuentros en sí (qué tipo de
tile puede dispararlos), que es un cambio más grande y no se hizo acá.

### Nido del Dragón y Guarida del Demonio — pieza central (05-09-2026)
`tree_up_3` y `tree_down_3` (las salas de los jefes) ahora muestran al
propio dragón/demonio como decoración central fija, usando sus mismos
sprites de batalla (`battle_dragon_ancestral`/`battle_demonio_menor`),
bien más grandes que un tile normal para que se sientan imponentes —
puramente visual, sigue siendo pasto alto (tile 1) alrededor lo que
dispara el encuentro real, no la decoración en sí. Con respaldo dibujado
a mano (círculo verde / triángulo rojo) si la textura no carga. Probado
con mock los 4 escenarios (con/sin textura, para cada uno) — los 4 pasan.
...
**✅ Actualización (04-09-2026): terreno base integrado — ítem 1 del roadmap prácticamente completo.**
Se recibió un paquete nuevo con hojas de terreno tipo autotile
(`tiles-earth-32x32.png`, `tiles-grass-medium-32x32.png`,
`tiles-terrain-32x32.png` para arena). Dado que son hojas complejas con
muchas variantes/bordes (pensadas para un sistema de blending completo,
un proyecto en sí mismo), se usó el mismo criterio ya probado con el
agua: un único recorte fijo de 32×32 por hoja, verificado visualmente
comparando varias posiciones candidatas (`(1,1)` resultó ser un relleno
sólido y parejo en las 3) antes de confiar en cualquiera — mismo cuidado
que evitó el problema anterior de un recorte mal elegido. Reemplaza el
dibujo a mano para camino (tile 0), pasto alto (tile 1) y arena (tile
7), con respaldo automático si la textura no carga. Probado con mock: 5
escenarios (los 3 tipos con textura real, los 3 sin ninguna cargada, y
confirmar que la entrada de cueva — que es un hito visual distintivo,
no un piso — no se ve afectada).

También llegó `cave-tiles-32x32.png` (piso de interior de cueva),
verificado como seguro pero sin integrar todavía — reservado para
cuando se diseñe un mapa de interior de cueva (mismo patrón que los
edificios), ya que la entrada de cueva visible desde afuera es un hito
visual (arco + boca oscura), no un tile de piso plano.
...
## ✅ Pared de edificio y mobiliario reales (05-09-2026)

Se recibió un kit de construcción modular grande (`Tiles/`: castillo,
ciudad, cueva, barco, mobiliario — más de 300 piezas individuales, cada
una su propio archivo, tamaños variables no uniformes a 32×32). Igual
que con el terreno, **no se implementó un sistema de ensamblaje
completo** (definir esquinas, bordes, conexiones entre piezas sería un
proyecto en sí mismo) — se eligieron 3 piezas puntuales, verificadas
individualmente, para reemplazar lo que ya estaba dibujado a mano:

- **Pared de edificio** (tile 9): `Tiles/CityInterior/CityInterior_2_1.png`
  (48×48, panel de piedra clara) — reemplaza `Black.png` como base;
  la ventana dibujada a mano se mantiene encima, sigue apareciendo en
  ~70% de los tiles.
- **Escritorio de recepción**: `Tiles/Furniture/Furniture_21_1.png`
  (16×16, mostrador con cajones).
- **Cama**: `Tiles/Furniture/Furniture_16_1.png` (16×32 — el doble de
  alta que ancha, se mantuvo esa proporción al escalar).

**Respaldo de 2 niveles para la pared** (probado con mock): si no existe
`tile_wall_panel`, cae a `tile_black` (ya integrado); si tampoco existe
eso, cae al rectángulo dibujado a mano de siempre. Cruz médica y
estrella policial siguen dibujadas a mano — no se identificó una pieza
del kit claramente destinada a esos símbolos específicos.

**Nota de transparencia real:** las piezas de `Furniture/` tienen fondo
transparente genuino (canal alfa, no color negro) — se verificó con
Python/PIL antes de confiar en la vista previa, que las mostraba sobre
negro por una limitación del visor, no del archivo.

**El resto del kit (300+ piezas) queda disponible sin usar** — quedó
guardado en `/tmp` durante esta sesión pero no se copió al proyecto
salvo las 3 piezas elegidas; si más adelante se quiere un sistema de
edificios más elaborado (fachadas de castillo completas, interiores de
barco, etc.), este kit ya está verificado como seguro y listo para esa
...
**✅ 7 de 8 combinaciones con arte real (04-09-2026) — ampliado de 3 a 4 opciones por género.**

Se recibieron `male/002.png`, `male/003.png`, `female/002.png`,
`female/003.png`, `female/004.png` — arte real distinto, reemplazando
los placeholders duplicados anteriores. Se amplió el sistema de 3 a 4
opciones por género (`APPEARANCE_PRESETS`/`PEOPLE_SPRITE_COMBOS` en
`CharacterVisual.js`, espejo en `resolve_appearance_preset()` de
`config.php` — confirmado programáticamente que ambos coinciden exacto).

**`male/004.png` no se integró:** el archivo se parecía a un diseño de
superhéroe con capa reconocible — mismo criterio de seguridad de
siempre. La opción 4 masculina sigue existiendo como elección válida
(cae al dibujo a mano automáticamente), solo falta un reemplazo
genuinamente original.

**`female/004.png` es un duplicado exacto de `female/002.png`** (mismo
archivo, mismo contenido) — se integró tal cual llegó, pero hoy elegir
la opción 2 o la 4 femenina se ve idéntico. Pendiente de confirmar si
fue intencional o un error al armar el paquete.

Probado con mock: las 7 texturas reales se cargan correctamente,
`people_male_4` correctamente NO se intenta cargar (cae al dibujo a
mano), y los 2 objetos de presets (boy/girl) tienen 4 opciones cada uno
para que el selector se muestre completo.
