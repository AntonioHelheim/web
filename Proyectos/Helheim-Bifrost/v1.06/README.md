...
## Pueblo Origen — el mapa principal, inspirado en Renca a escala real

El mapa principal (`overworld` en el código) se llama **Pueblo Origen**
— está inspirado en la comuna de **Renca**, en el sector norponiente de
Santiago de Chile, pero de aquí en adelante "Pueblo Origen" es el nombre
oficial del lugar para cualquier efecto (lo que ve el jugador en
pantalla, documentación, etc.); "Renca" queda como la referencia real
que inspiró su diseño.
...
- **El gran árbol** ahora en la **esquina superior izquierda** exacta del
  mapa (9×10 tiles — se redujo 1 fila el 05-09-2026 para abrir una
  entrada real hacia sus 6 niveles interiores, ver más abajo).
...
│   ├── graphics-catalog.json  # Catálogo de graphics — ver estado real archivo por archivo adentro
│   ├── npc-spawns.json        # Posiciones de NPCs ambientales por mapa (data-driven, ítem 4)
│   └── map-encounters.json    # Pools de encuentros salvajes por mapa (Gran Árbol/Cueva, ítem 4)
├── graphics/                # Archivos .png reales, solo los verificados individualmente como seguros
│   ├── animations/            # bkgindex1/2.png, clouds.png (fondo de index.php)
│   ├── builds/                 # doors1-3.png, object_boulder.png, object_tree.png, water.png, Flowers1-2.png, Black.png
│   └── characters/people/     # male/001.png, female/001.png (2 de 6 combinaciones)
├── js/
...
│   │   ├── NPC.js                 # Personajes ambientales: máquina de estados IDLE/PATROL (ítem 7)
...
│   ├── test-npc-wander.js              # Merodeo de NPCs contra los mapas reales — ROADMAP ítems 4, 7, 10
│   ├── test-tile-rendering.js          # Terreno/edificios/decoración: textura real + respaldo — ROADMAP ítem 1
│   ├── test-character-visuals.js       # Validación de sprite, 8 combinaciones, escala por género
│   ├── test-preload-regression.js      # Regresión: no perder characterCreated si el mapa guardado es inválido
...
1. Crea la base de datos e importa el esquema:
   ...
   ```
2. **Credenciales de base de datos** — si tu MySQL local no usa
   `root`/sin contraseña, o si vas a subir esto a un hosting real:
   copia `api/config.local.example.php` como `api/config.local.php` y
   completa ahí tus datos reales (`DB_HOST`/`DB_NAME`/`DB_USER`/`DB_PASS`).
   **Este archivo es tuyo — nunca te lo vuelvo a entregar ni a
   sobreescribir en futuras versiones del proyecto** (está en
   `.gitignore`), así que tus credenciales reales sobreviven sin
   importar cuántas veces actualices el resto del código. No edites
   `api/config.php` directamente para poner credenciales reales: ese
   archivo sí se regenera en cada entrega, y perderías el cambio.
...
- Caminar hasta cualquier borde de Pueblo Origen (el mapa principal) te lleva a la
  ruta de ese punto cardinal (Norte, Sur, Este u Oeste), y viceversa para
  volver.
- El gran árbol está en la esquina superior izquierda de Pueblo Origen (ahí
...
- Al sur del gran árbol hay 3 edificios (Comisaría, Hospital, Hotel) —
  cada uno con un interior simple (una habitación con alguien atendiendo)
  al entrar por la puerta.
- El Gran Árbol tiene una entrada real (puerta en su base) que lleva a
  6 niveles interiores (3 hacia arriba, 3 hacia abajo) — cada uno con
  sus propias criaturas salvajes (Battlers, y un dragón como guardián de
  la cima). La Cueva de Don Emilio también es accesible ahora (antes
  era solo decorativa) y lleva a 2 niveles con sus propias criaturas
  (un demonio guarda el fondo del árbol, no de la cueva).
