...
<h2>1b. Configuración local (api/config.local.php)</h2>
<?php $configLocalExiste = is_file(__DIR__ . '/api/config.local.php'); ?>
<pre class="<?= $configLocalExiste ? 'ok' : 'muted' ?>"><?php
echo $configLocalExiste
    ? "Existe — tus credenciales de api/config.local.php tienen prioridad sobre los valores genéricos de api/config.php.\nDB_HOST actual: " . DB_HOST . "\nDB_NAME actual: " . DB_NAME . "\nDB_USER actual: " . DB_USER
    : "No existe todavía — se están usando los valores genéricos de api/config.php (probablemente incorrectos en un hosting real).\nSi la conexión a la base de datos falla después de actualizar el proyecto, esta es la causa más probable:\ncopia api/config.local.example.php como api/config.local.php y pon ahí tus credenciales reales — ese archivo nunca se vuelve a sobreescribir en futuras entregas.";
?></pre>
