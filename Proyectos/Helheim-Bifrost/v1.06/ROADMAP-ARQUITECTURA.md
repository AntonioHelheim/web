...
**✅ Prácticamente completo (04-09-2026) — todos los tipos de tile con textura real, salvo la entrada de cueva (que es un hito visual, no un piso).**
...
**✅ Avanzado (05-09-2026) — terreno, personajes (7/8), edificios, y 10 criaturas de combate integrados. Solo Audio/ sigue sin archivos.**
...
`package.json` (0 dependencias) corre las 8 suites con un solo comando:
`npm test` → `check-maps.js` + `test-battle-rules.js` +
`test-npc-wander.js` + `test-tile-rendering.js` +
`test-character-visuals.js` + `test-preload-regression.js` (Node) +
`test-wild-battles.php` + `test-csrf-and-rate-limit.php` (PHP). Las 3
últimas de Node (05-09-2026) consolidan pruebas que antes vivían como
scripts sueltos en `/tmp` durante el desarrollo — nada las corría
automáticamente, así que una regresión futura en terreno/personajes/la
restauración de sesión no habría sido detectada por `npm test`.
