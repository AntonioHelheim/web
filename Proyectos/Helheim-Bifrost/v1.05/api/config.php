...
// Antes de usar cualquier valor por defecto (los de abajo son
// genéricos/placeholder — se sobreescriben cada vez que se entrega una
// versión nueva del proyecto): si existe api/config.local.php, se
// carga primero. Ese archivo NUNCA se entrega ni se vuelve a generar —
// es tuyo, vive solo en tu servidor, y sobrevive a cualquier actualización
// futura del proyecto. Ahí es donde van tus credenciales reales de
// producción (o cualquier ajuste local que no quieras perder). Ver
// api/config.local.example.php para la plantilla y las instrucciones.
...
function map_encounters(): array
{
    static $encuentros = null;
    if ($encuentros === null) {
        $ruta = __DIR__ . '/../data/map-encounters.json';
        $leido = json_decode((string) @file_get_contents($ruta), true);
        $encuentros = is_array($leido) ? $leido : [];
    }
    return $encuentros;
}

function random_monster(?string $mapKey = null): array
{
    $catalog = species_catalog();
    // Si el mapa actual tiene un pool específico de encuentros (ej. los
    // niveles del Gran Árbol o la Cueva de Don Emilio), se elige solo entre
    // esas — si no, se mantiene el comportamiento de siempre (cualquier
    // especie del catálogo completo). Se filtran las claves que no
    // existan en el catálogo por seguridad (nunca debería pasar, pero
    // evita un error si data/map-encounters.json queda desincronizado).
    $encuentros = map_encounters();
    $keysDisponibles = array_keys($catalog);
    if ($mapKey !== null && isset($encuentros[$mapKey])) {
        $pool = array_values(array_intersect($encuentros[$mapKey], $keysDisponibles));
        if (count($pool) > 0) {
            $keysDisponibles = $pool;
        }
    }
    $key = $keysDisponibles[array_rand($keysDisponibles)];
    $base = $catalog[$key];
    return [
        'speciesKey' => $key,
        'name' => $base['name'],
...
{
    // Espejo exacto de APPEARANCE_PRESETS en js/entities/CharacterVisual.js
    // — si cambias algo acá, cambia también ahí. Ampliado de 3 a 4
    // opciones el 04-09-2026.
    $tabla = [
        'boy' => [
            ...
            4 => ['skinColor' => '#e8b878', 'hairColor' => '#6b3410', 'eyeColor' => '#1a3a5c'],
        ],
        'girl' => [
            1 => ['skinColor' => '#f1c27d', 'hairColor' => '#2c1b18', 'eyeColor' => '#3b2415'],
            2 => ['skinColor' => '#f1c27d', 'hairColor' => '#e8c268', 'eyeColor' => '#274b8f'],
            3 => ['skinColor' => '#8d5524', 'hairColor' => '#1a1a1a', 'eyeColor' => '#2f6b3a'],
            4 => ['skinColor' => '#701848', 'hairColor' => '#1a0a10', 'eyeColor' => '#3a1020'],
        ],
    ];
