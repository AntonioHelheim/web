...
class PreloadScene extends Phaser.Scene {
  ...
  preload() {
    ...
    // Pared de edificio real, y mobiliario real (escritorio, cama) —
    // reemplazan el dibujo a mano de drawBuildingWall()/drawDecoration()
    // en TileVisuals.js, con el mismo respaldo automático de siempre.
    this.load.image('tile_wall_panel', `graphics/builds/tile-wall-panel.png?v=${v}`);
    this.load.image('furniture_desk', `graphics/builds/furniture-desk.png?v=${v}`);
    this.load.image('furniture_bed', `graphics/builds/furniture-bed.png?v=${v}`);
    // Criaturas de combate (Battlers/dragón/demonio, 05-09-2026) — un
    // solo cuadro estático (pose de reposo) por ahora, no animación
    // completa; reemplaza el círculo/rectángulo de color en
    // BattleScene.js cuando existe, ver buildCombatantVisual().
    const battlers = ['glenys', 'hex', 'ley', 'linail', 'oratio', 'ouzo', 'prime', 'wyvera'];
    battlers.forEach((nombre) => {
      this.load.image(`battle_battler_${nombre}`, `graphics/characters/battlers/${nombre}.png?v=${v}`);
    });
    this.load.image('battle_dragon_ancestral', `graphics/characters/dragon-battle.png?v=${v}`);
    this.load.image('battle_demonio_menor', `graphics/characters/littledemon-battle.png?v=${v}`);
    // Terreno base (camino/pasto/arena) — imágenes de tipo autotile
    // completas, se usa solo un recorte fijo ya verificado (ver
    // drawRealTerrainTile en TileVisuals.js).
    this.load.image('tile_terrain_earth', `graphics/builds/tiles-earth-32x32.png?v=${v}`);
    this.load.image('tile_terrain_grass', `graphics/builds/tiles-grass-medium-32x32.png?v=${v}`);
    this.load.image('tile_terrain_sand', `graphics/builds/tiles-terrain-32x32.png?v=${v}`);
    this.load.image('tile_terrain_cave_floor', `graphics/builds/cave-tiles-32x32.png?v=${v}`);
  }
