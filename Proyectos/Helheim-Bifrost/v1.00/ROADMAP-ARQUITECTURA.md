...
usar `graphics/autotiles` (ya catalogado en `data/graphics-catalog.json`,
plan detallado en `PLAN-GRAPHICS-AUDIO.md`): sin esto, esas imágenes no
se pueden aprovechar bien.
...
**Avance (30 y 31-08-2026):** ya tengo el listado completo de `audio/`
(27 archivos) y `graphics/` (43 archivos) — ambos catalogados en
`data/audio-events.json` y `data/graphics-catalog.json` (ver
`data/README.md`), con la convención de grilla 4×4 documentada. El plan
técnico detallado de CÓMO se implementa cada categoría (autotiles,
puertas animadas, personajes, árboles con etapas de vida, fondo de
`index.php`) vive en **[PLAN-GRAPHICS-AUDIO.md](./PLAN-GRAPHICS-AUDIO.md)**.

**Ya implementado sin esperar los archivos:** el cambio de jugabilidad de
apariencia (elegir 1 de 3 opciones preestablecidas por género, ya no
color libre) — es forward-compatible con los sprites reales de
`Characters/people/` sin tener que volver a tocar la base de datos.

**Lo único que falta de verdad:** los archivos `.ogg`/`.png` reales. Sin
ellos no se puede escribir el loader real ni probarlo (mismo principio
de "probar con datos reales" que seguimos en todo el proyecto) —
cualquier código de carga que se escriba antes fallaría en silencio o,
peor, parecería funcionar sin estarlo probado de verdad.
...
- *Riesgo/esfuerzo:* Medio — el catálogo y el plan ya están escritos;
  queda la implementación en sí, que sí depende de tener los archivos.
- *Depende de:* los archivos reales de `graphics/`/`audio/` subidos al
  proyecto (nombres y rutas ya confirmados, dimensión exacta en píxeles
  de cada cuadro pendiente de ver el archivo real).
