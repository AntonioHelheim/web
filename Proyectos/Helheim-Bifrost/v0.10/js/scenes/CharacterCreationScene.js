...
class CharacterCreationScene extends Phaser.Scene {
  ...
  init(data) {
    ...
    this.nextData = data;
    // editMode: true cuando se abre desde el menú en pleno juego (MenuScene),
    // en vez de la primera vez que se crea el personaje.
    this.editMode = !!data.editMode;
    this.initialAppearance = data.appearance || null;
  }

  create() {
    console.log('[Bifrost] CharacterCreationScene.create(), editMode =', this.editMode, 'initialAppearance =', this.initialAppearance);
    const seed = this.initialAppearance || {
      gender: 'boy', skinColor: SKIN_TONES[1], hairColor: HAIR_COLORS[0], eyeColor: EYE_COLORS[0],
    };
    this.skinIndex = Math.max(0, SKIN_TONES.indexOf(seed.skinColor));
    this.hairIndex = Math.max(0, HAIR_COLORS.indexOf(seed.hairColor));
    this.eyeIndex = Math.max(0, EYE_COLORS.indexOf(seed.eyeColor));
    this.appearance = {
      gender: seed.gender,
  ...
  cancelEdit() {
    console.log('[Bifrost] cancelEdit()');
  ...
  async confirm() {
    console.log('[Bifrost] confirm() llamado, editMode =', this.editMode, 'transitioning =', this.transitioning);
    ...
    try {
      ...
      console.log('[Bifrost] save_appearance.php respondió OK');
    } catch (err) {
      console.log('[Bifrost] save_appearance.php falló:', err);
    }

    if (this.editMode) {
      console.log('[Bifrost] editMode true -> resume OverworldScene con nueva apariencia');
      this.scene.stop();
      this.scene.resume('OverworldScene', { newAppearance: this.appearance });
      return;
    }
