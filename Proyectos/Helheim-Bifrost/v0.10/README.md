...
`game.php` e `index.php` cargan todos los `.js`/`.css` con un parámetro
`?v=...` (`$ASSET_VERSION`, arriba del todo en cada archivo). **Cada vez que
subas una versión nueva del proyecto, cambia ese valor** (por ejemplo, a la
fecha del día) — si no, el navegador (o a veces el propio hosting) puede
seguir sirviendo la versión vieja en caché de esos archivos, y vas a estar
probando código desactualizado sin darte cuenta aunque el servidor ya tenga
los cambios nuevos. Si algo "no cambia" después de subir una actualización,
antes de asumir que algo sigue roto: sube el valor de `$ASSET_VERSION` y
haz un refresco forzado del navegador (Ctrl+Shift+R / Ctrl+F5), o prueba en
...
>
> También se agregó una guarda contra doble-clic/doble-envío en los botones
> de `MenuScene` y en `CharacterCreationScene.confirm()` (evita que un clic
> doble dispare dos transiciones de escena a la vez, dejándolas en un
> estado inconsistente).
>
> **¿Sigue sin funcionar después de esto?** Antes de seguir cambiando
> código: sube `$ASSET_VERSION` en `game.php`/`index.php` (ver la nota de
> caché arriba del todo) y haz un refresco forzado del navegador. Si el
> problema persiste, abre las herramientas de desarrollador (F12) → pestaña
> "Console", reproduce el error, y fíjate si aparece algo en rojo — ese
> mensaje exacto es la forma más rápida de encontrar la causa real.
>
> **Logs de diagnóstico temporales:** para ayudar a encontrar el problema
> sin poder probarlo yo mismo en un navegador real, agregué mensajes
> `console.log('[Bifrost] ...')` en cada paso del flujo del menú (abrir,
> cada botón, cambiar de escena, reanudar el mapa). Con la consola abierta
> (F12) deberías ver una secuencia como:
> ```
> [Bifrost] Tecla M detectada, abriendo menú
> [Bifrost] pauseForOverlay -> MenuScene
> [Bifrost] MenuScene.create()
> [Bifrost] Cambiar apariencia clickeado, actionTaken = false
> [Bifrost] CharacterCreationScene.create(), editMode = true ...
> [Bifrost] confirm() llamado, editMode = true ...
> [Bifrost] save_appearance.php respondió OK
> [Bifrost] editMode true -> resume OverworldScene con nueva apariencia
> [Bifrost] OverworldScene resumed, data = {newAppearance: {...}}
> ```
> Si la secuencia se corta en algún punto (o no aparece nada), ese es
> exactamente el paso que está fallando — cópiame la consola completa
> (incluyendo cualquier línea en rojo) y lo resolvemos directo. Son logs
> inofensivos, se pueden borrar más adelante sin afectar nada.
