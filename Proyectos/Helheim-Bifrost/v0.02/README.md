...
correo (`email` NULL) hasta que le agregues uno — revisa la nota de
migraciones más abajo (`sql/005_email_login.sql`).
...
textura). Lo que sí hice: expandir el mapa muchísimo más que antes, para
que las distancias entre hitos se sientan genuinamente amplias y a escala
entre sí, representando un sector de Renca en vez del 1:1 literal completo:

| | Antes | Ahora |
|---|---|---|
| Tamaño | 28×20 (560 tiles) | **100×72 (7.200 tiles)** |
| Área real aprox. | 49m × 35m | **175m × 126m** |
| Objetos de Phaser (terreno) | ~2.000 | ~16.500 |

(El área creció 12.9×, pero los objetos a dibujar solo ~8×, gracias a que
`drawPath()` ahora dibuja menos brizas de pasto por tile — el camino liso
es la mayoría del mapa nuevo, así que decorar cada uno habría multiplicado
demasiado la cantidad de objetos. Si notas que va lento en tu máquina,
avísame: el siguiente paso sería cambiar a un sistema de tilemap con
textura en vez de formas dibujadas una por una, mucho más eficiente para
mapas grandes, pero es un cambio de arquitectura más grande.)

**Detalles reales incorporados:**
- **El gran árbol** en el extremo norte (ahora de 10×10 tiles ≈ 17.5m —
  del tamaño real de un árbol grande y viejo), cerca de donde estarían
  calles como El Raval, cercanas a El Montijo y Vicuña Mackenna.
- **Los Cerros de Renca**, una franja montañosa mucho más grande y creíble
  al noroeste (en la realidad, cerca del 25% del territorio de Renca son
  cerros) — coincide con el límite norte real de la comuna, con Quilicura.
...
- **El río Mapocho**, ahora una banda ancha (7 tiles ≈ 12m, un río urbano
...
calcen con las nuevas puertas del pueblo, mucho más separadas entre sí
ahora que el mapa es más grande.

Verifiqué con el mismo chequeo de conectividad que uso para todos los
mapas (flood-fill desde el punto de aparición) que el 100% de las 5.365
celdas transitables del nuevo pueblo son alcanzables, y que las 4 rutas
siguen conectando correctamente en ambos sentidos.
...
│   ├── config.php             # Conexión PDO (entorno auto) + helpers compartidos
...
└── sql/
    ...
    └── 005_email_login.sql               # Migración: login por código (email)
```
...
> - `sql/005_email_login.sql` — columna `email` + tablas de login por código.
>   **Después de importarlo, tus cuentas viejas no van a poder iniciar
>   sesión hasta que les agregues un correo** (ver el detalle en la
>   sección "Login por código" más arriba).
>
...
- Caminar hasta cualquier borde de Renca (el mapa principal) te lleva a la
  ...
  volver.
- El gran árbol está en el extremo norte de Renca — se puede caminar todo
  alrededor de él. Al noroeste hay un cerro rocoso con dos cuevas (una es
  la Cueva de Don Emilio), y el río Mapocho cruza todo el extremo sur del
  mapa (con un puente transitable justo frente a la salida sur).
