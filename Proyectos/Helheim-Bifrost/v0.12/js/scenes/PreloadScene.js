...
class PreloadScene extends Phaser.Scene {
  ...
  preload() {
    ...
    // Sprites de personaje reales, uno por combinación género+preset que
    // ya tengamos subida (ver PLAN-GRAPHICS-AUDIO.md). Las combinaciones
    // sin archivo real siguen usando el dibujo a mano
    // (buildCharacterVisual) como respaldo — Player.js elige
    // automáticamente cuál usar según lo que haya logrado cargar acá, así
    // que no hace falta esperar a tener las 6 (3 presets × 2 géneros)
    // para empezar a usar las que ya existen.
    const v = window.BIFROST_ASSET_VERSION || '';
    this.load.spritesheet('people_male_1', `graphics/characters/people/male/001.png?v=${v}`, { frameWidth: 32, frameHeight: 48 });
  }

  async create() {
    SPECIES = this.cache.json.get('speciesData') || {};
    // Se registran las 4 animaciones de caminata de cada sprite real que
    // se haya logrado cargar arriba — no hace nada si la textura no
    // existe, así que es seguro llamarla aunque todavía falten sprites.
    defineCharacterAnimations(this, 'people_male_1');
