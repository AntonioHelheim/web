...
try {
    ...
    // Límite de retos enviados: máximo 10 por minuto, para que no se pueda
    // usar esto para hostigar a otros jugadores con retos en cadena.
    $reciente = $pdo->prepare(
        'SELECT COUNT(*) FROM battle_challenges
         WHERE from_user_id = ? AND created_at >= (NOW() - INTERVAL 1 MINUTE)'
    );
    $reciente->execute([$userId]);
    if ((int) $reciente->fetchColumn() >= 10) {
        respond(['error' => 'Estás enviando retos demasiado rápido. Espera un momento.'], 429);
    }
