...
el de mayor impacto visual). El ítem 6 (pipeline completo) ya no está
100% bloqueado: la parte de audio quedó catalogada en
`data/audio-events.json`, solo falta la parte visual y los archivos
reales de ambas carpetas.

**Resueltos:** ítem 2 (30-08-2026, `js/core/battleRules.js`), ítem 3
(30-08-2026, `api/wild_battle_start.php` + `api/wild_battle_action.php`),
ítem 4 (30-08-2026, `data/species.json` + `data/battle-rules.json`), e
ítem 5 (30-08-2026, CSRF en 9 endpoints + rate-limiting en
`challenge_send.php`).

**Catalogado, a la espera de archivos reales:** audio (27 archivos,
ítem 6 parcial) — ver `data/audio-events.json`.

**Se pueden empezar ya, sin esperar nada:** la parte de testing de mapas
del ítem 10 (ya tenemos precedente de "pruebas de fuego" reales tras
cerrar los ítems 4 y 5 — vale la pena formalizarlas en un script, no solo
correrlas a mano cada vez), ítem 7 y 8 (aunque siguen bloqueados por no
haber NPCs/contenido narrativo todavía — mejor esperar a que lleguen los
sprites de `graphics/characters` y `graphics/trainers`).

Mi sugerencia concreta para seguir: **formalizar el testing de mapas**
(parte del ítem 10) — es de las pocas tareas que quedan con riesgo
prácticamente nulo y ya tenemos el patrón (flood-fill de conectividad)
probado muchas veces a mano; guardarlo como script reutilizable evita que
cada rediseño de mapa dependa de que me acuerde de correrlo manualmente.
Aparte de eso, quedamos a la espera del listado de `graphics/` para
poder avanzar la parte de mayor impacto visual (ítems 1 y 6).
