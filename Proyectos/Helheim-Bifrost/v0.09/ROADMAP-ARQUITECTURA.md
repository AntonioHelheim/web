...
**🔶 Parcialmente resuelto (30-08-2026) — la parte de mapas.**
...
`package.json` nuevo (0 dependencias, solo scripts) permite correr todo
con `npm test` (`check-maps.js` + `test-battle-rules.js` juntos).

**Lo que falta** (fuera de alcance de esta victoria fácil): testing real
de combate/API contra una base de datos — ya se hizo manualmente varias
veces con SQLite en memoria (ítems 3, 4, 5) pero no quedó guardado como
script reutilizable en el repo. Sería el siguiente paso natural si se
quiere seguir en esta línea.

- *Por qué importa:* *"El sistema debe disponer de logging suficiente
  para detectar errores... testing unitario, integración."*
- *Riesgo/esfuerzo:* Bajo para la parte de conectividad de mapas (ya
  resuelta). Más alto para testing real de combate/API contra base de
  datos, que requeriría guardar los arneses de prueba usados durante los
  ítems 3-5 como scripts reutilizables en vez de archivos temporales.
...
el de mayor impacto visual). El ítem 6 (pipeline completo) sigue a la
espera de esa misma lista para la parte visual — audio ya quedó
catalogado en `data/audio-events.json`.

**Resueltos:** ítem 2 (30-08-2026), ítem 3 (30-08-2026), ítem 4
(30-08-2026), ítem 5 (30-08-2026).

**Parcialmente resuelto:** ítem 10 (30-08-2026) — `scripts/check-maps.js`
+ `package.json` (`npm test`). Falta la parte de testing de combate/API
contra base de datos (usar los arneses de los ítems 3-5 como base).

**Catalogado, a la espera de archivos reales:** audio (ítem 6 parcial).

Como quedamos: la próxima vez que hablemos me vas a pasar el listado de
`graphics/` y sus subcarpetas — con eso puedo destrabar los ítems 1 y 6,
que son los de mayor impacto visual. Mientras tanto, lo siguiente con
menor riesgo/esfuerzo disponible sería guardar los arneses de prueba de
combate/API (ítems 3-5) como scripts reutilizables en `scripts/`, para
terminar de cerrar el ítem 10 — puedo avanzar eso en la próxima sesión si
prefieres seguir progresando en paralelo mientras preparas el listado.
